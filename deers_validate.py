...
def run(cfg, rank, dev_dataset_file, device, model, news_title):
    ...
    with torch.no_grad():
        ...
        with open(cfg.result_path + 'tmp_small_deers_{}.json'.format(rank), 'w', encoding='utf-8') as f:
...
def gather(cfg, turn, validate=False):
    ...
    for i in range(filenum):
        with open(output_path + 'tmp_small_deers_{}.json'.format(i), 'r', encoding='utf-8') as f:
    ...
    with open(cfg.result_path + 'tmp_deers_{}.json'.format(turn), 'w', encoding='utf-8') as f:
        json.dump(tmp_dict, f)
...
def main(cfg):
    ...
    model = DEERS(model_cfg)

    saved_model_path = os.path.join('./checkpoint/', 'deers.ep{0}'.format(cfg.epoch))
    ...
    gather_all(cfg.result_path, 1, file_num, validate=True, save=True, tmp_name='tmp_deers')
